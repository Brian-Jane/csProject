...
class TasksWindow(QtWidgets.QWidget):
    def __init__(self, T:Tasks, taskobject:taskobject=None):
        ...
        for i,colour in self.T.fetchFolders():
            folders.append((i,colour))

        self.folder=QtWidgets.QComboBox()

        for text,color in folders:
            if not text:
                self.folder.addItem(text)
            else:
                self.folder.addItem(text)
                self.folder.setItemData(self.folder.count() - 1, color, role=QtCore.Qt.UserRole)

        self.mainLayout.addWidget(self.folder,2,1)
